...
class ReportsController extends Controller
{
    ...
    /**
     * Show a single completed appointment record.
     * Returns JSON for AJAX modal population (includes status history).
     *
     * Route:  GET /receptionist/reports/{id}
     * Named:  receptionist.reports.show
     */
    public function show(Request $request, int $id)
    {
        ...
        if (! $record) {
            abort(404, 'Completed appointment record not found.');
        }
    ...
    /**
     * Permanently delete a completed appointment record.
     *
     * Route:  DELETE /receptionist/reports/{id}
     * Named:  receptionist.reports.destroy
     */
    public function destroy(Request $request, int $id)
    {
        ...
        if (! $appointment) {
            return redirect()
                ->route('receptionist.reports.index', $this->preserveFilters($request))
                ->with('error', 'Record not found or is not a completed appointment.');
        }
        ...
        return redirect()
            ->route('receptionist.reports.index', $this->preserveFilters($request))
            ->with('success', 'Appointment record #' . $id . ' has been permanently deleted.');
    }
    ...
    private function resolveDateRange(string $filter, ?string $customDate): array
{
    ...
    private function preserveFilters(Request $request): array
    {
        return array_filter([
